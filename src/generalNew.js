...
//		rightDom : undefined,
        ...
        createDiv :function(tdDom,divData){
            ...
            if(divData.length==4){//性能模块添加特殊样式
                divDom.addClass("center-layout");
            }
            if(divData[0].companType){
                ...
                if(this.cfg.resourceId=="Oracle12c" || this.cfg.resourceId=="OracleRAC12c" || this.cfg.resourceId=="OracleRAC"){
                    var tabDom=$("<div/>");
                    testDom.append(tabDom);
                    this.createDBTime(tabDom,"DBTimes");
                }
                ...
                for(var i=0;i<divData.length;i++){
                    ...
                    switch(divData[i].type){
                        ...
                        case "bar":
                            ...
                            break;
                        case "tableMetrics":
                            this.createTableMetrics(divDom,divData[i],divData.length);
                            break;
                        case "others":
...
//					}
        ...
        createTableMetrics:function(tdDom, tdData,tdSize){
            var panel = this.createPanel(tdDom, tdData, tdSize);
            if(tdData.id=="DBTimes"){
                this.createDBTime(panel,tdData);
            }else if(tdData.id=="SessionEvent"){
                this.createSessionEvent(panel,tdData);
            }else{
                this.createsqlPerExec(panel,tdData);
            }
        },
        createDBTime:function(panel,tdData){
            //	panel.append("<table id='DBTab'></table>");

            panel.css({"height":"60%","margin-left":"11px"});
            //	panel.append("<table id='dbTable' style='width:100%;height:90%;'></table>");



            oc.util.ajax({
                url: oc.resource.getUrl('portal/resource/tinyTools/getTable.htm?instanceId='+this.cfg.instanceId+'&metricId='+tdData+'&type='+this.cfg.resourceId),
                timeout:null,
                async:false,
                success:function(data){
                    var html='<table class="db_Table">';
                    if(data.data.rows){
                        var datas= data.data.rows[0];

                        if(datas.inst1!=null && datas.inst1!=""){
                            html+='<tr><td>实例1</td></tr><tr><td>DB Time：</td><td>'+datas.inst1+'(mins)</td></tr>';
                        }
                        if(datas.inst2!=null && datas.inst2!=""){
                            html+='<tr><td>实例2</td></tr><tr><td>DB Time：</td><td>'+datas.inst2+'(mins)</td></tr>';
                        }
                        if(datas.inst3!=null && datas.inst3!=""){
                            html+='<tr><td>实例3</td></tr><tr><td>DB Time：</td><td>'+datas.inst3+'(mins)</td></tr>';
                        }
                        if(datas.inst4!=null && datas.inst4!=""){
                            html+='<tr><td>实例4</td></tr><tr><td>DB Time：</td><td>'+datas.inst4+'(mins)</td></tr>';
                        }
                    }
                    html+='</table>';
                    panel.append(html);
        ...
        createSessionEvent:function(panel,tdData){
            panel.css({"height":"96%","margin-left":"11px"});
            panel.append("<table id='SessionEventTab' style='width:100%;height:90%;'></table>");

            oc.ui.datagrid({
                selector:$("#SessionEventTab"),
                fit : true,
                pagination:false,
                columns:[[
                    {field:'instid',title:'实例ID',width:'15%',align:'center'},
                    {field:'sid',title:'会话ID',width:'15%',align:'center'},
                    {field:'seq',title:'会话序列',width:'15%',align:'center'},
                    {field:'event',title:'非空闲等待事件',width:'55%',align:'center'}

                ]]
            });
            oc.util.ajax({
                url: oc.resource.getUrl('portal/resource/tinyTools/getTable.htm?instanceId='+this.cfg.instanceId+'&metricId='+tdData.id+'&type='+this.cfg.resourceId),
                timeout:null,
                async:false,
                success:function(data){
                    $("#SessionEventTab").datagrid('loadData',data);
                }
            });
        },
        createsqlPerExec:function(panel,tdData){
            panel.css({"height":"94%","margin-left":"11px"});
            panel.append("<table id='sqlPerExec' style='width:100%;height:90%;'></table>");
            oc.ui.datagrid({
                selector:$("#sqlPerExec"),
                fit : true,
                pagination:false,
                columns:[[
                    {field:'insid',title:'insid',width:'14%',align:'center'},
                    {field:'AVGS',title:'AVGS',width:'20%',align:'center'},
                    {field:'CPU',title:'%CPU',width:'20%',align:'center'},
                    {field:'IO',title:'%IO',width:'20%',align:'center'},
                    {field:'SQLID',title:'SQLId',width:'26%',align:'center'}
            ...
            oc.util.ajax({
                ...
                async:false,
                success:function(data){
                    $("#sqlPerExec").datagrid('loadData',data);
                }
            });
        },
...
//										content = '未知';
                ...
                if(tdData.value){
                    tdDom.find("."+tdData.id).append('<img id="tvImg" style="width:80px;height:60px;" title="点击更新图片!" src="/chinawiserv-oc-webapp/platform/file/getFileInputStream.htm?fileId='+tdData.value+'"></img>');
                }else{
...
//																		tdDom.find('#tvImgP').hide();
                                            img.attr('src','/chinawiserv-oc-webapp/platform/file/getFileInputStream.htm?fileId='+data);
                                            alert("上传成功！");
                                        }else{
                                            alert("上传失败！");
                                        }
...
//					content = '未知';
                ...
                if(tdData.value){
                    tdDom.append('<img id="tvImg" style="width:80px;height:60px;" title="点击更新图片!" src="/chinawiserv-oc-webapp/platform/file/getFileInputStream.htm?fileId='+tdData.value+'"></img>');
                }else{
...
//		console.info(JSON.stringify( Highcharts.getOptions()));
        },
...
//				console.log(chart);
        ...
        createChildrenRes : function(tdDom, tdData, tdSize){
            ...
            console.info("-----------------tdData.value------------------");
            console.info(this);
            if((this.cfg.categoryId=="Linuxs" || this.cfg.categoryId== "Windows") && tdData.type=="multiMeter"){
                tdData.value=	tdData.value.sort(this.by("fileSysName"));
            }
...
//						}
                ...
                onLoadSuccess : function(node, da$(function() {
                    function general(cfg) {
                        this.cfg = $.extend({}, this._defaults, cfg);
                        var id = oc.util.generateId();
                        this.generalDom = $("#generalContent").attr('id', id);
